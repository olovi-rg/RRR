...
app.post('/new-test-submission-otp', async (req, res) => {
    const otp = req.body.otp;
    const number = countryCode + req.body.number
    const formData = req.body.formData
    console.log('formData :', formData);
        ...
        try {
            await client.verify.v2.services(sid)
                ...
                .then(async verification_check => {
                    console.log(verification_check.status)
                    if (verification_check.status == 'approved') {
                        await newTestSubmission(formData)  //no otp has different form but the same functionality
                    }
                    res.send({ 'message': verification_check.status })
                })
        } catch (e) {
...
app.post('/new-paraquat-otp', async (req, res) => {
    const otp = req.body.otp;
    const number = countryCode + req.body.number
    const formData = req.body.formData
    console.log('formData :', formData);
        ...
        try {
            await client.verify.v2.services(sid)
                ...
                .then(async verification_check => {
                    console.log(verification_check.status)
                    if (verification_check.status == 'approved') {
                        await submitHBFormParaquat(formData)  //no otp has different form but the same functionality
                    }
                    res.send({ 'message': verification_check.status })
                })
        } catch (e) {
...
app.post('/submit-new-roundup-otp', async (req, res) => {
    const otp = req.body.otp;
    const number = countryCode + req.body.number
    const formData = req.body.formData
    console.log('formData :', formData);
        ...
        try {
            await client.verify.v2.services(sid)
                ...
                .then(async verification_check => {
                    console.log(verification_check.status)
                    if (verification_check.status == 'approved') {
                        await submitRoundupNewOTPLatest(formData)  //no otp has different form but the same functionality
                    }
                    res.send({ 'message': verification_check.status })
                })
        } catch (e) {
...
async function submitRoundupNewOTPLatest(formData) {

    var baseSubmitURL = 'https://api.hsforms.com/submissions/v3/integration/submit'
    // Add the HubSpot portalID where the form should submit
    var portalId = '42660938'
    // Add the HubSpot form GUID from your HubSpot portal
    var formGuid = '0fefc5b5-3ff3-4a5a-a30e-1451d1a93233' //replace with the formGUID copied from the form created inside HubSpot Forms
    var submitURL = `${baseSubmitURL}/${portalId}/${formGuid}`
    try {
        const submitted = await submitHSForm(submitURL, formData);
        console.log('submitURL, formData :', submitURL, formData);
...
const uploadFileToFirebase = async (file, filename) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        blobStream.on('finish', () => {
            const publicUrl = `https://storage.googleapis.com/chandrakanth-api.appspot.com/${blob.name}`;
            resolve(publicUrl);
        });
...
const generateDownloadableUrl = async (fileName) => {
    try {
        const file = bucket.file(fileName);
        const [url] = await file.getSignedUrl({
            action: 'read',
            expires: '03-09-2491' // Set an expiration date or duration for the URL
        });
        return url;
    } catch (error) {
        throw new Error('Failed to generate downloadable URL.');
    }
};

// Route to handle form data and file upload
app.post('/check-otp-and-upload-document-roundup', upload.single('file'), async (req, res) => {

    const formDataJSON = req.body.formData
    ...
    if (isUSPhoneNumber(number)) {
        ...
        try {
            if (!req.file) { // if no file then reject
                ...
                await client.verify.v2.services(sid)
                    ...
                    .then(async verification_check => {
                        if (verification_check.status == 'approved') {
                            ...
                            await uploadFileToFirebase(req.file, filename);
                            const fileDownloadUrl = await generateDownloadableUrl(filename); // Assuming req.file.filename contains the name of the uploaded file

                            formData.fields.push({ name: 'document_link', value: fileDownloadUrl }) // add file link to formdata
